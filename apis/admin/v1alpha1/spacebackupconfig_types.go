...
type SpaceBackupObjectStorage struct {
	...
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:XValidation:rule="self.source != 'Secret' || (has(self.secretRef) && self.secretRef.name != '' && self.secretRef.namespace != '')",message="secretRef.name and namespace must be set when source is Secret"
	// +kubebuilder:validation:XValidation:rule="self.source != 'Environment' || (self.environment != nil && self.environment.name != '')",message="environment.name must be set when source is Environment"
