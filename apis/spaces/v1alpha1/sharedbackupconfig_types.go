...
type BackupObjectStorage struct {
	...
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:XValidation:rule="self.source != 'Secret' || (has(self.secretRef) && self.secretRef.name != '')",message="secretRef.name must be set when source is Secret"
